...
test_that('it should use the default calendar', {
  ...
  expect_error(bizdays('2013-01-02', '2090-01-07', cal),
               'Given date out of range.')
  expect_error(bizdays('1700-01-02', '2013-01-07', cal),
               'Given date out of range.')
})

test_that('it should create a short calendar and test its boundaries', {
  cal <- Calendar_(start.date = '2013-01-01', end.date = '2013-12-31',
                   weekdays = c('saturday', 'sunday'))
  expect_true(cal$start.date == '2013-01-01')
  expect_true(cal$end.date == '2013-12-31')
  expect_true(all(cal$weekdays == c('saturday', 'sunday')))
  expect_equal(bizdays('2013-01-02', '2013-01-03', cal), 1)
  expect_error(bizdays('2013-01-02', '2014-01-02', cal),
               'Given date out of range.')
  expect_error(bizdays('2012-01-02', '2013-01-02', cal),
               'Given date out of range.')
})
...
test_that('it should work with unordered calendars', {
  ...
  expect_equal(bizdays('2013-07-12', '2014-07-12', cal1),
...
test_that('it should create non financial calendars', {
  create.calendar(name = "test", weekdays = c('saturday', 'sunday'),
                  financial = FALSE)
...
test_that('it should warn for bad settings', {
  data(holidaysANBIMA)
  expect_warning(Calendar_(holidaysANBIMA),
...
test_that('it should pass NULL calendar', {
  expect_error(adjust.next(as.Date('2013-01-01'), NULL),
               'Given calendar is NULL')
  expect_error(preceding(as.Date('2013-01-01'), NULL),
               'Given calendar is NULL')
  expect_error(bizdays('2013-08-13', '2025-01-02', NULL),
               'Given calendar is NULL')
...
if (requireNamespace("RQuantLib", quietly = TRUE)) {
  test_that('it should check if QuantLib calendars have been loaded', {
    ...
    load_quantlib_calendars(c('Argentina', 'UnitedStates/NYSE'), '2016-01-01',
                            '2016-12-31')
  ...
  test_that('it should check if QuantLib calendars behave correctly', {
    ...
    ql_bd <- RQuantLib::businessDaysBetween('Argentina', as.Date('2016-01-01'),
                                            as.Date('2016-07-12'))
    expect_equal(bizdays('2016-01-01', '2016-07-12', 'QuantLib/Argentina'),
                 ql_bd)

    ql_bd <- RQuantLib::businessDaysBetween('Argentina', as.Date('2016-01-01'),
                                            as.Date('2016-07-10'))
    expect_equal(bizdays('2016-01-01', '2016-07-10', 'QuantLib/Argentina'),
                 ql_bd)

    ql_bd <- RQuantLib::businessDaysBetween('UnitedStates/NYSE',
                                            as.Date('2016-01-01'),
                                            as.Date('2016-07-10'))
...
if (requireNamespace("timeDate", quietly = TRUE)) {
  ...
  test_that('it should check if QuantLib calendars behave correctly', {
    ...
    expect_equal(is.bizday('2016-01-01', 'Rmetrics/LONDON'),
                 as.logical(timeDate::isBizday(timeDate::timeDate('2016-01-01'),
                                               timeDate::holidayLONDON(2016)))
    )

    expect_equal(is.bizday('2016-07-10', 'Rmetrics/LONDON'),
                 as.logical(timeDate::isBizday(timeDate::timeDate('2016-01-10'),
                                               timeDate::holidayLONDON(2016)))
    )

    expect_equal(is.bizday('2016-07-12', 'Rmetrics/LONDON'),
                 as.logical(timeDate::isBizday(timeDate::timeDate('2016-01-12'),
                                               timeDate::holidayLONDON(2016)))
    )

    expect_equal(is.bizday('2016-12-31', 'Rmetrics/LONDON'),
                 as.logical(timeDate::isBizday(timeDate::timeDate('2016-12-31'),
                                               timeDate::holidayLONDON(2016)))
    )

    load_rmetrics_calendars(2017)
    expect_equal(is.bizday('2017-01-01', 'Rmetrics/LONDON'),
                 as.logical(timeDate::isBizday(timeDate::timeDate('2017-01-01'),
                                               timeDate::holidayLONDON(2017)))
    )

    expect_equal(is.bizday('2017-12-31', 'Rmetrics/LONDON'),
                 as.logical(timeDate::isBizday(timeDate::timeDate('2017-12-31'),
                                               timeDate::holidayLONDON(2017)))
    )
  })
}
